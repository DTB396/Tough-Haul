...
function scanFile (filePath) {
  ...
  lines.forEach((line, index) => {
    ...
    const lowOpacityWhite = line.match(/color:\s*rgb\s*\(\s*255\s*,\s*255\s*,\s*255\s*,\s*(0\.[0-6])\s*\)/)
    if (lowOpacityWhite) {
    ...
    const lowOpacity = line.match(/color:\s*rgb\(a?\)?\s*\(\s*(\d+)\s*,\s*(\d+)\s*,\s*(\d+)\s*,\s*(0\.\d+)\s*\)/)
    if (lowOpacity) {
